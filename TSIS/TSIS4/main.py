...
ASSETS = os.path.join(BASE_DIR, "snake_assets")

# ── Assets — загружаем если есть, иначе None ──────────────────
...
background = _load_img("background.png", (WIDTH, HEIGHT))
death_img   = _load_img("skyrim_death.jpg", (WIDTH, HEIGHT))
eat_sound   = _load_sound("eat.mp3")
death_sound = _load_sound("death.mp3")
...
def draw_bg():
    """Рисует фон — картинку если есть, иначе тёмный цвет."""
    if background:
...
def play_sound(sound, settings):
    """Играет звук только если включён в настройках и файл загружен."""
...
def generate_food(snake, obstacles):
    ...
    if random.random() < 0.20:
        return {
            "type": "poison", "pos": pos, "weight": 0,
            "color": DARK_RED,
            "spawn_time": pygame.time.get_ticks(), "lifetime": 8000,
        }
    ...
    return {
        "type": "normal", "pos": pos, "weight": weight,
        "color": color,
        "spawn_time": pygame.time.get_ticks(), "lifetime": lifetime,
    }
...
def generate_power_up(snake, obstacles, food):
    ...
    return {
        "type": p_type, "pos": pos, "color": colors[p_type],
        "spawn_time": pygame.time.get_ticks(), "lifetime": 8000,
    }
...
def safe_obstacle_position(pos, snake_head):
    hx, hy = snake_head
    px, py = pos
    return abs(hx - px) + abs(hy - py) >= CELL * 3
...
def generate_obstacles(level, snake, food_pos):
    ...
    while len(obstacles) < count and attempts < 500:
        attempts += 1
        pos = (random.randrange(CELL, WIDTH  - CELL, CELL),
               random.randrange(CELL, HEIGHT - CELL, CELL))
...
def main_menu(username):
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit(); sys.exit()
            if play_btn.clicked(event): return "play"
            if lead_btn.clicked(event): return "leaderboard"
            if set_btn.clicked(event):  return "settings"
...
def leaderboard_screen():
    ...
    while True:
        ...
        if error:
            ...
            draw_text("#   Username        Score   Level   Date",
                      SMALL_FONT, GREEN, 35, 85)
            y = 120
...
def settings_screen(settings):
    ...
    while True:
        ...
        draw_text(f"Grid: {'ON' if settings['grid'] else 'OFF'}",
                  SMALL_FONT, WHITE, 90,  210)
        draw_text(f"Sound: {'ON' if settings['sound'] else 'OFF'}",
                  SMALL_FONT, WHITE, 245, 210)
        draw_text("Snake color:", SMALL_FONT, WHITE, 405, 210)
        pygame.draw.rect(SCREEN, tuple(settings["snake_color"]),
                         (450, 240, 50, 30))
...
def game_over_screen(username, score, level, best):
    ...
    # Показываем death_img если есть
    if death_img:
    ...
    while True:
        # Перерисовываем фон каждый кадр
        if death_img:
            SCREEN.blit(death_img, (0, 0))
        else:
            SCREEN.fill((40, 0, 0))
...
def play_game(username, settings):
    ...
    while True:
        ...
        if collision:
            if shield:
                shield   = False
                new_head = snake[0]   # стоим на месте
            else:
                play_sound(death_sound, settings)
                if not saved:
                    try:
                        if db: db.save_result(username, score, level)
                    except Exception:
                        pass
                    saved = True
        ...
        if power_up is None and now - last_power_try > 1000:
            last_power_try = now
        ...
        if power_up and new_head == power_up["pos"]:
            if power_up["type"] == "speed":
                power_active, power_end_time = "speed", now + 5000
            elif power_up["type"] == "slow":
                power_active, power_end_time = "slow",  now + 5000
        ...
        if   power_active == "speed": current_speed = base_speed + 5
        elif power_active == "slow":  current_speed = max(3, base_speed - 4)
        ...
        for obs in obstacles:
            pygame.draw.rect(SCREEN, GRAY,
                             (obs[0], obs[1], CELL - 1, CELL - 1))

        pygame.draw.rect(SCREEN, food["color"],
                         (food["pos"][0], food["pos"][1], CELL - 1, CELL - 1))

        if power_up:
            pygame.draw.rect(SCREEN, power_up["color"],
                             (power_up["pos"][0], power_up["pos"][1],
                              CELL - 1, CELL - 1))
        ...
        draw_text(f"Shield: {'ON' if shield else 'OFF'}",
                  SMALL_FONT, YELLOW if shield else WHITE, 420, 8)

        if power_active:
            left = max(0, (power_end_time - now) // 1000)
            draw_text(f"Power: {power_active} {left}s",
                      SMALL_FONT, BLUE, 10, HEIGHT - 28)
